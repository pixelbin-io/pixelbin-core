...
exports.processParams = exports.checkPresetOnly = exports.isPreset = exports.flattenTrnsfrmPattern = exports.getPartsFromUrl = exports.getPatternFromTransformations = exports.rgbHex = exports.getUnArchivedPresets = exports.getObjFromUrl = exports.getUrlFromObj = void 0;
...
const getUrlFromObj = function (obj, config) {
    ...
    if (obj.isCustomDomain && obj.cloudName) {
        throw new PixelbinErrors_1.PDKIllegalArgumentError("key cloudName is not valid for custom domains");
    }
    ...
    if (obj.worker && typeof obj.workerPath !== "string")
        throw new PixelbinErrors_1.PDKIllegalArgumentError("key workerPath should be a defined");
    if (obj.worker) {
        obj["pattern"] = "wrkr";
    }
    else {
        obj["pattern"] =
            (0, exports.getPatternFromTransformations)(obj["transformations"], config) || "original";
    }
...
const getObjFromUrl = function (url, config, flatten) {
    const parts = (0, exports.getPartsFromUrl)(url, config);
    try {
...
const getPartsFromUrl = function (url, config) {
    const parts = (0, common_utils_1.getUrlParts)(url, config);
...
exports.getPartsFromUrl = getPartsFromUrl;
...
function flattenTrnsfrmPattern(pattern, presets) {
    ...
    transformations.forEach((ele) => {
        ...
        if (ele.startsWith("p:")) {
            ...
            if (!presetParams.n) {
                throw new PixelbinErrors_1.PDKTransformationError(`Missing param 'n' for preset: ${ele}`);
            }
        ...
        if (preset) {
            if (presets[preset]) {
                ...
                throw new PixelbinErrors_1.PDKTransformationError(`Invalid preset: ${preset}`);
            }
        }
        else {
...
exports.flattenTrnsfrmPattern = flattenTrnsfrmPattern;
...
function flattenPreset(preset, presetConfig) {
    ...
    if (isVariableSupport) {
        ...
        if (paramsConfigKeyList.length <= 0) {
            throw new PixelbinErrors_1.PDKTransformationError(`Params config of preset ${presetName} is empty`);
        }
                ...
                prstParamsKeyList.forEach((prstParamsKey) => {
                    ...
                    if (variableConfig) {
                        ...
                        if (!validators[variableConfig.type](patternParamsObj[prstParamsVal], variableConfig)) {
                            throw new PixelbinErrors_1.PDKTransformationError(`Value for '${prstParamsKey}' in '${presetName}' must be a type of ${variableConfig.type}`);
                        }
                        // replace $variable_name with val
                        transformationStr = transformationStr.replace(`$${prstParamsVal}`, patternParamsObj[prstParamsVal]);
                    }
                    else {
                        throw new PixelbinErrors_1.PDKTransformationError(`Variable object not defined for the variable $${prstParamsKey}`);
                    }
                });
            }
        });
 ...
 * @returns {boolean}
 */
const checkPresetOnly = (pattern) => {
    const transformations = pattern.split(OPERATION_SEPARATOR);
    return transformations.every(exports.isPreset);
};
exports.checkPresetOnly = checkPresetOnly;
